...
def get_plot(data, queried_data):
    fig = go.Figure()
    if not queried_data:
        ...
        for i in range(5):
            ...
            for k in range(len(queried_data[i])):
                x2.append(tag[i])
                data2.append(queried_data[i][k])
...
def get_all_data():
    engine = create_engine(
        'mysql+pymysql://admin:root1234@boxplot.co5oyjgpvi8x.us-east-1.rds.amazonaws.com:3306/boxplot', max_overflow=10)
...
def start():
    try:
        ...
        return render_template('index.html', graph=get_plot(data, query_data), query_sentence=s)
    except:
